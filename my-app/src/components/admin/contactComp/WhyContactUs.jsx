...
export default function WhyContact() {
  ...
  useEffect(() => {
    siteConfigApi
      ...
      .catch((err) => {
        console.warn("Failed to load Why Contact us data:", err);
        toast.error("Could not load Why Contact Us data: " + err.message);
      })
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error(e.message || "Failed to save Why Contact Us content");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionDesignManager sectionKey="whyContactUs" sectionLabel="Why Contact Us" />
    </div>
  );
}