...
export default function BrandLogoUploader({ value, onChange }) {
  ...
  const handleFile = async (file) => {
    ...
    setUploading(true);
    try {
      const url = await uploadImage(file, "brands-logos");
      onChange(url);
    ...
    <div className="space-y-3">
      ...
      {value ? (
        <div className="relative w-32 h-32 border rounded-xl overflow-hidden group">
          <img src={value} alt="Brand logo" className="w-full h-full object-contain" />
          <button
            type="button"
            onClick={removeLogo}
