// src/components/public/ClientTestimonialsSlider.jsx
"use client";
...
export const defaultTestimonialsConfig = {
  ...
  headerBadgeText: "What Our Clients Say",
  headingClass: "mt-2 text-3xl md:text-4xl font-black tracking-tight",
  headingText: "Trusted By Industry Leaders",
  ...
  authorBadgeLabel: "Client", // now always "Client"
  ...
  readAllText: "Read All Clients Testimonials →",
  readAllHref: "/testimonials/clients", // adjust route as needed
};

// ----- Component with props merging into default config -----
export default function ClientTestimonialsSlider({ config = {} }) {
  ...
  useEffect(() => {
    ...
    Promise.all([testimonialsApi.getPublic(), clientsApi.getPublic()])
      .then(([t, c]) => {
        if (!alive) return;
        // Keep only client testimonials
        const items = (Array.isArray(t) ? t : [])
          .filter(item => item.sourceType === "CLIENT" || item.client);
        setTestimonials(items);
        // logos state removed – if you need logos for a marquee or similar, you can still compute them here
      })
      .catch(() => {
        if (!alive) return;
        setTestimonials([]);
  ...
  const currentLogo = current?.client?.logo;
  const currentHref = current?.client ? `/clients/${current.client.slug}` : null;
